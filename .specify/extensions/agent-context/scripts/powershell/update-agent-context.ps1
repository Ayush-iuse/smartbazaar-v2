...
if ($null -eq $Options) {
    ...
    foreach ($candidate in @('python3', 'python')) {
        if (Get-Command $candidate -ErrorAction SilentlyContinue) {
            try {
                ...
                # Ignore execution alias or launcher failures and continue
            }
        }
    }
    ...
    if ($pythonCmd) {
        try {
            $pyScript = Join-Path $ProjectRoot '.specify/extensions/agent-context/scripts/powershell/temp_yaml_parser.py'
            $pyCode = @'
import json
import sys
try:
    ...
    print("agent-context: PyYAML is required to parse extension config; cannot update context.", file=sys.stderr)
    sys.exit(2)
...
try:
    ...
    print(f"agent-context: unable to parse {sys.argv[1]} ({exc}); cannot update context.", file=sys.stderr)
    sys.exit(2)
...
'@
            [System.IO.File]::WriteAllText($pyScript, $pyCode, (New-Object System.Text.UTF8Encoding($false)))
